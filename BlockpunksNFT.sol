...
// File contracts/BlockpunksNFT.sol





contract BlockpunksNFT is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    uint public _tokenSupply = 7500;
    ...
    mapping(uint => bool) _isMinted;
    mapping(address => bool) _isAllowedToMint;
    mapping(address => uint) _mintedTokenOf;

    
    event TokenMinted( address _to, uint _id);
    ...
    /**
    * @dev Transfer token ownership from one address to another address
    */
    function mint(uint _amount) public {
        require(_mintedTokenOf[msg.sender] + _amount <= 5, "You have already minted the maximum amount of tokens");
        require(_totalSupply < _tokenSupply, "Token supply exceeded");
        require(_totalSupply + _amount <= _tokenSupply, "Not enough tokens left");

        for (uint index = 0; index < _amount; index++) {
            uint _id = _tokenIds.current();
            _ownerOf[_id] = msg.sender;
            _isMinted[_id] = true;
            _mintedTokenOf[msg.sender]++;
    ...
    */
    ...
    /**
    * @dev get the tokenURI by given tokenId
    */
    function tokenURI(uint _tokenId) public view override returns (string memory){
        require(_isMinted[_tokenId] == true, "Token not minted yet");
        return string(abi.encodePacked(_baseTokenURI, _tokenId, _baseExtension));
    }
